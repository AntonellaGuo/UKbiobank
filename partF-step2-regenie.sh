...
# - /Data/urticaria_results_1.loco.gz - from part D
# - /Data/urticaria_results_pred.list - from part D

# Additional inputs
# for each chromosome, you will run a separate worker
# - /{exome_file_dir}/ukb23155_c1_v2.bed
# - /{exome_file_dir}/ukb23155_c1_v2.bim 
# - /{exome_file_dir}/ukb23155_c1_v2.bam 
...
for chr in {1..22}; do
  run_regenie_cmd="regenie --step 2 --bed ukb${data_field}_c${chr}_v2 --out assoc.c${chr}\
    ...
    --phenoCol urticaria_cc --covarCol age --covarCol sex --covarCol ethnic_group --covarCol ever_smoked \
    --pred urticaria_results_pred.list --bsize 200\
    --pThresh 0.05 --minMAC 3 --threads 16 --gz"

  dx run swiss-army-knife \
   -iin="${exome_file_dir}/ukb${data_field}_c${chr}_v2.bed" \ #bed file for chromosome
   -iin="${exome_file_dir}/ukb${data_field}_c${chr}_v2.bim" \ #bim file for chromosome
   -iin="${exome_file_dir}/ukb2${data_field}_c${chr}_v2.fam"\ #fam file for chromosome
   -iin="${data_file_dir}/WES_c${chr}_snps_qc_pass.snplist"\ #individual QC snplist produced with 04-step2-qc-filter.sh
   -iin="${data_file_dir}/urticaria_wes.phe" \  #phenotype results
